...
The original [rtv](https://github.com/michael-lazar/rtv) is no longer maintained, but I've been using it even tho there are other forks of it. However I've been missing a few things from it, which is what this fork hopes to achieve. I haven't actually checked out what new features other forks provide if any, but I just felt I needed to make this fork.

### Main differences

Right now the only difference is posts are now in the `.md` file type instead of `.txt` and the header, banner and footer have been removed, most of which will be configurable later on.
